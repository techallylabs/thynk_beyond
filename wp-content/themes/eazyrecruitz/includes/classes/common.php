...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				...
				'layout'            => eazyrecruitz_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),

				'sidebar_type'      => eazyrecruitz_meta( 'sidebar_source_type', $page_id, 'd' ),

				'sidebar_elementor' => eazyrecruitz_meta( 'sidebar_elementor_template', $page_id ),

				'sidebar'           => eazyrecruitz_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),

				'banner_type'       => eazyrecruitz_meta( 'banner_source_type', $page_id ),

				'banner_elementor'  => eazyrecruitz_meta( 'banner_elementor_template', $page_id ),

				'banner'            => eazyrecruitz_set( eazyrecruitz_meta( 'banner_page_background', $page_id ), 'url' ),

				'title'             => eazyrecruitz_meta( 'banner_banner_title', $page_id ),

				'text'             => eazyrecruitz_meta( 'banner_banner_text', $page_id ),
				
				'enable_banner'     => eazyrecruitz_meta( 'banner_page_banner', $page_id ),
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : eazyrecruitz_the_title( $template ),
				
				'text'         => $options->get( $text ) ? $options->get( $text ) : eazyrecruitz_the_title( $template ),
				
				'banner'        => eazyrecruitz_set( $bg, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$return = [
			...
			'text'             => ($text) ? $text : esc_html__('Reliable & Cost Efficient Recruitment Agency Based On Los Angeles', 'eazyrecruitz'),
